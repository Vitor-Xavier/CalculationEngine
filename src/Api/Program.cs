...
namespace Api {
    public class Program {
        ...
        static async Task Main (string[] args) {
            ...
            Evento fatorG = new Evento {
                ...
                se (@Fisico.AreaEdificada > 0.0) {
                    valor = @Fisico.AreaEdificada * 1.05 * COALESCE(@FisicoOutros[0].Crc, @Fisico.crc, 0);
                } senao {
                    valor = @Fisico.AreaEdificada * @Fisico.Testada * @FisicoOutros[0].Crc;
                }
                retorno valor * 10;"
            };
            ...
            Parallel.ForEach (first.Value, new ParallelOptions { MaxDegreeOfParallelism = 1 }, item => {
                ...
                foreach (var m in auxMemory) {
                    ...
                    foreach (var v in m.Values) {
                        ...
                        .Select (x => new { Key = $"@{m.Table}[{i}].{x.Key}", Value = new GenericValueLanguage (x.Value) })
                            .ToList ().ForEach (item => {
        ...
        private static async Task<List<object>> GetData (string sql, IEnumerable<string> colunas) {
            ...
            try {
                connection = new SqlConnection ("Data Source=192.168.0.132,1433;initial catalog=SMARTB_PMBotucatu;persist security info=True;user id=smar; password=smarapd;");
        ...
        private static List<TabelaColuna> ListaTabelaColuna (IList<IToken> Tokens, IDictionary<string, int> TokenTypeMap) {
            ...
            grupo = tokensTypeVarTableColuna.Select (x => new {
                    tabela = x.Text.RemoveCaracter ("[0]").RemoveCaracter ("@").SubstringWithIndexOf ('.'),
